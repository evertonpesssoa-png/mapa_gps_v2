...
// ======================================
// FUNÇÃO PARA BUSCAR POIs MANUAIS PRÓXIMOS
// ======================================

async function buscarPOIsManuais(lat, lng, type = 'all', radius = 2000) {
  ...
  const withinRadius = poisComDistancia.filter(poi => poi.distance <= radius);
  
  // Ordenar por distância (mais perto primeiro)
  withinRadius.sort((a, b) => a.distance - b.distance);
  
  console.log(`📌 POIs manuais: ${withinRadius.length} encontrados dentro de ${radius}m`);
...
// ======================================
// FUNÇÃO PARA CARREGAR TODOS OS POIs NO MAPA
// ======================================

async function carregarPOIsManuaisNoMapa(lat, lng, radius = 5000, mapLayer) {
  if (!mapLayer || !lat || !lng) return [];
  
  const pois = await buscarPOIsManuais(lat, lng, 'all', radius);
  ...
  console.log(`🗺️ ${pois.length} POIs manuais carregados no mapa`);
  return pois;
}
